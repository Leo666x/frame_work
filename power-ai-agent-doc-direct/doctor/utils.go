...
// 初始化配置参数
func (a *DoctorAgent) InitReportConfig(rsp *server.AgentResponse, event *server.SSEEvent, StrConfig map[string]interface{}) {
...
// StrictMatchDoctor 在候选列表中寻找名字完全一致的医生
func StrictMatchDoctor(targetName string, candidates []map[string]interface{}) []map[string]interface{} {
...
func (a *DoctorAgent) ReadKnowledge(req *server.AgentRequest, query, className, matchField string, outputFields []string, topK int) ([]milvus_mw.SearchResult, error) {
...
func (a *DoctorAgent) LlmCall(req *server.AgentRequest, prompt string) (string, error) {
...
func (a *DoctorAgent) GetRedis(c *gin.Context, req *server.AgentRequest) *powerai.SessionValue {
...
func (a *DoctorAgent) GetHistoryDialogue(req *server.AgentRequest) (string, string, error) {
	// 1 准备历史对话
...
func (a *DoctorAgent) RerankDeal(searchResult [][]milvus_mw.SearchResult, enterpriseId, query, matchField string) ([]milvus_mw.SearchResult, error) {
...
func (a *DoctorAgent) Updatememory(
	req *server.AgentRequest,
	session *powerai.SessionValue,
	aiMsg string, // AI 本次生成的文本回复 (用于历史记录和 Layer3 判断)
	mySlots *DoctorSlots, // 当前最新的私有状态
) error {
	agentKey := "power-ai-agent-doc-direct" // 或者使用 a.App.Manifest.Code

	// ====================================================
	// 1. 同步私有状态 (Struct -> Session Map)
	// ====================================================

	// 将当前最新的 mySlots 结构体赋值回 Session 的 Map 中
	session.GlobalState.AgentSlots[agentKey] = mySlots

	// ====================================================
	// 2. 更新流程控制上下文 (FlowContext)
	// ====================================================

	// 更新当前活跃 Agent (确保 Layer 3 知道是谁在服务)
	session.FlowContext.CurrentAgentKey = agentKey

	// 如果 aiMsg 为空（例如只发了卡片），尝试用 resp.Data 中的描述兜底，防止 Layer 3 判空
	if aiMsg == "" {
		session.FlowContext.LastBotMessage = "[医生卡片信息]"
	} else {
...
// LoadAgentSlots 泛型辅助函数：将 AgentSlots 中的原始数据加载到具体的私有结构体中
//
// 参数:
//
//	slotsMap: session.GlobalState.AgentSlots
//	agentKey: 智能体的唯一标识 (如 "power-ai-agent-doc-direct")
//	target:   目标结构体的指针 (e.g., &DoctorSlots{})
//
// 返回: error
